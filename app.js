...
const prompts = [
  ...
  "If tonight had a soundtrack, what would the opening song be?",
  "What makes a dinner unforgettable?",
  "What small detail changes the mood of an evening?",
  "What city has stayed with you the longest?",
...
function findGuestByName(input) {
  ...
  return guests.find((guest) => normalize(guest.name) === q);
}

function showGuestPage(guest) {
...
function returnToLanding() {
  ...
  landingName.value = "";
  landingMessage.textContent = "Enter your full name to open your personal page.";
  window.scrollTo({ top: 0, behavior: "smooth" });
}
...
nameForm.addEventListener("submit", (event) => {
  ...
  const guest = findGuestByName(landingName.value);

  if (!guest) {
    landingMessage.textContent = "We couldn't find that name. Please try your full name.";
    return;
  }

  landingMessage.textContent = `Welcome ${guest.name}. Opening your page...`;
  showGuestPage(guest);
});
